...
test-init-image:
	...
	docker run --rm --cpus 1 --name pid pid1runner ls /
	docker run --rm --cpus 1 --name pid pid1runner id
	docker run --rm --cpus 1 --name pid pid1runner --workdir=/home  pwd
	docker run --rm --cpus 1 --name pid pid1runner --env HELLO=WORLD --env=FOO=BYE printenv HELLO FOO
